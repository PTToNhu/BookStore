...
const CartUser = () => {
    ...
    const fetchBooks = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/api/cart/${customerId}`
        );
        if (!response.ok) {
  ...
  const handleDecreased = async (bookID) => {
    ...
    if (currentNumOfBooks) {
      ...
      try {
        const respone = await fetch("http://localhost:3000/api/cart/add", {
          method: "POST",
          headers: {
  ...
  const handleDelete = async (BookID) => {
    try {
      const respone = await fetch("http://localhost:3000/api/cart/delete", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
